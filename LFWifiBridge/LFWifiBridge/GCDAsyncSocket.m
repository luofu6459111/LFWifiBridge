...
- (NSUInteger)readLengthForTermWithHint:(NSUInteger)bytesAvailable shouldPreBuffer:(BOOL *)shouldPreBufferPtr
{
	...
	if (shouldPreBufferPtr) //shouldPreBufferPtr是BOOL类型的指针
	{
		NSUInteger buffSize = [buffer length];
		NSUInteger buffUsed = startOffset + bytesDone;
...
- (NSUInteger)readLengthForTermWithPreBuffer:(GCDAsyncSocketPreBuffer *)preBuffer found:(BOOL *)foundPtr
{
	...
	uint8_t *buf = (uint8_t *)[buffer mutableBytes] + startOffset + bytesDone - bufLen;//将指针指向buffer的某个位置，从这个位置开始匹配，term的最后一个字节正好是prebuffer的第一个字节
 ...
 * 
 * If the term is found, the number of excess bytes after the term are returned.找到term后，term后面多余的字节要进行处理，这里返回多余的字节数
...
- (void)doReadEOF
{
	...
	else
	{
		if (![self usingCFStreamForTLS])
		{
			// Suspend the read source (if needed)  对端已经不会写东西了，所以挂起读
